...
// MARK: - Global Modifier Key Observer
...
struct MainWindow: View {
    ...
    @StateObject private var keyboardMonitor = ModifierKeyMonitor()
...
private struct ShardListRow: View {
    let title: String
    let modeName: String
    let iconName: String
    let isPinned: Bool
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: isCompact ? 2 : 4) {
            ...
            HStack(spacing: 6) {
                ...
                // Compact mode: show tag icons in title row
    ...
    private func tagSummaryChip(for tag: (name: String, symbol: String, colorHex: String), showsLabel: Bool) -> some View {
        HStack(spacing: 2) {
            Image(systemName: tag.symbol)
...
private struct TagPopoverContent: View {
    let shard: Shard
    let availableTags: [Tag]
    var accentColor: Color = Color(red: 79/255, green: 70/255, blue: 229/255)
                ...
                VStack(spacing: 8) {
                    ...
                    HStack(spacing: 8) {
                        TextField("SF Symbol", text: $quickTagSymbol)
                            .textFieldStyle(.roundedBorder)
                            .font(.body)
                        ColorPicker("", selection: $quickTagColor).labelsHidden()
                    }
                    Button("Create & Attach") {
                        onCreateTag(
                            quickTagName.trimmingCharacters(in: .whitespacesAndNewlines),
                            quickTagSymbol.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "tag.fill" : quickTagSymbol,
...
private struct ProtectionPrompt: Identifiable {
    enum Purpose {
        case protect
        case unlockProtected
        case removeProtection
    ...
    var title: String {
        switch purpose {
        case .protect:
            return "Protect Shard"
        case .unlockProtected:
            return "Unlock Protected Shard"
        case .removeProtection:
    ...
    var description: String {
        switch purpose {
        case .protect:
            return "This password will be required before the shard content can be viewed."
        case .unlockProtected:
            return "Enter the shard password to reveal the content for this session."
        case .removeProtection:
            return "Enter the shard password to remove its dedicated protection."
        }
    }
}
